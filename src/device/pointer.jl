...
on-device counterpart of `CuPtr`, additionally keeping track of the address space
...
export vloada
@inline @generated function vloada(::Type{Vec{N, T}}, ptr::CUDA.DevicePtr{T, AS}, i::Integer = 1) where {N, T, AS}
    alignment = sizeof(T) * N
    vec_len = (sizeof(T) * N) ÷ sizeof(Float32)

    return quote
        vec_ptr = convert(CUDA.DevicePtr{NTuple{$vec_len, VecElement{Float32}}, AS}, ptr)
        return unsafe_load(vec_ptr, (i - 1) ÷ N + 1, Val($alignment))
    end
end

export vstorea!
@inline @generated function vstorea!(::Type{Vec{N, T}}, ptr::CUDA.DevicePtr{T, AS}, x, i::Integer = 1) where {N, T, AS}
    alignment = sizeof(T) * N
    vec_len = (sizeof(T) * N) ÷ sizeof(Float32)
